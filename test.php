...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkEmailStmt = $conn->prepare("SELECT email FROM t_users WHERE email = ? OR username = ?");
    $checkEmailStmt->bind_param("ss", $email, $username);
    $checkEmailStmt->execute();
    $checkEmailStmt->store_result();

    if ($checkEmailStmt->num_rows > 0) {
        $message = "Email and/or username already registered.";
        $toastClass = "#007bff"; // Primary color
    } else {
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO t_users (username, email, password) VALUES (?, ?, ?)");
