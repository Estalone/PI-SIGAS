...
if ("POST" === $_SERVER["REQUEST_METHOD"]) {
  ...
  if (empty($user_name) || empty($user_pwd) \vert{}\vert{} empty($user_email) || empty($user_type)) {$err = "All fields are required.";
  } elseif ($user_pwd !== $user_repwd) {$err = "Passwords do not match.";
  } else {
    // Hash da senha
    ...
    if ($supermode && "dm" === $user_type) {$sql .= " OR user_type = ?";
    }
    ...
    if ($stmt = $conn->prepare($sql)) {
      ...
      while ($row = $result->fetch_assoc()) {$check++;
      ...
      if ($check > 0) {
        ...
        if ($stmt_ins = $conn->prepare($sql_ins)) {
          $stmt_ins->bind_param("ssssis", $user_name, $user_type,$user_email, $user_pwd_hash,$user_status, $user_token);$stmt_ins->execute();
